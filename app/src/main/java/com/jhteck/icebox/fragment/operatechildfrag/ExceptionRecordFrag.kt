...
import com.jhteck.icebox.databinding.AppFragmentOperaExcepitionBinding
...
/**
 *@Description:(异常操作记录页面Fragment)
 *@author wade
 *@date 2023/6/28 21:59
 */
class ExceptionRecordFrag : BaseFragment<AccountViewModel, AppFragmentOperaExcepitionBinding>() {
    ...
    companion object {
        fun newInstance(): ExceptionRecordFrag {
            return ExceptionRecordFrag()
        }
    }
    ...
    override fun createViewBinding(
        ...
        container: ViewGroup?
    ): AppFragmentOperaExcepitionBinding {
        return AppFragmentOperaExcepitionBinding.inflate(layoutInflater)
    }
    ...
    override fun initView() {
        ...
        val selectList = listOf("全部",OperationErrorEnum.REBOOT.desc, OperationErrorEnum.TIME_OUT.desc, OperationErrorEnum.NOT_MARK.desc)
                ...
                override fun onItemSelected(
                    ...
                    if (position != lastSelectPosition) {
                        ...
                        when (position) {
                            ...
                            1 -> updateUIbyRfidOperationList(filterByOperation(OperationErrorEnum.REBOOT.v))
                            2 -> updateUIbyRfidOperationList(filterByOperation(OperationErrorEnum.TIME_OUT.v))
                            3 -> updateUIbyRfidOperationList(filterByOperation( OperationErrorEnum.NOT_MARK.v))
                        }
                    }
                }
    ...
    private fun filter(keyWord: String): List<ExceptionRecordBean> {
        // 过滤原本的列表，返回一个新的列表
        val filterList = mutableListOf<ExceptionRecordBean>()

        for (l in rfidOperationList!!) {
            if (l.error_code_desc.contains(keyWord) || l.error_code_desc.contains(keyWord))
                filterList.add(l)
        }
        return filterList
    }

    private fun filterByOperation(keyWord: Int): List<ExceptionRecordBean> {
        // 过滤原本的列表，返回一个新的列表
        val filterList = mutableListOf<ExceptionRecordBean>()
        for (l in rfidOperationList!!) {
            if (l.error_code_desc.toInt() == keyWord)
                filterList.add(l)
        }
        return filterList
    }
    ...
    private fun updateUIbyRfidOperationList(list: List<ExceptionRecordBean>) {
    ...
    private var rfidOperationList = mutableListOf<ExceptionRecordBean>()
    override fun initObservables() {
        super.initObservables()
        if (bindingInitialzed()) {
            viewModel.onExceptionRecordLoad.observe(this) {
    ...
    private fun listSortBy(index: Int, tv: TextView) {
        var sortByList: List<ExceptionRecordBean>? = null
