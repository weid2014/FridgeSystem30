...
import com.jhteck.icebox.face.CompareResult;
import com.jhteck.icebox.face.FaceHelper;
import com.jhteck.icebox.face.FaceServer;
import com.jhteck.icebox.face.callback.OnRegisterFinishedCallback;
import com.jhteck.icebox.face.constants.LivenessType;
import com.jhteck.icebox.face.constants.RecognizeColor;
import com.jhteck.icebox.face.constants.RequestFeatureStatus;
import com.jhteck.icebox.face.util.ConfigUtil;
import com.jhteck.icebox.face.util.FaceRectTransformer;
import com.jhteck.icebox.face.widget.FaceRectView;
...
public class RecognizeViewModel extends ViewModel implements RecognizeCallback {
    ...
    private void registerFace(final byte[] nv21, FacePreviewInfo facePreviewInfo) {
        ...
        registerNv21Disposable = Observable.create((ObservableOnSubscribe<Boolean>) emitter -> {
            ...
            int res = registerEngine.init(BaseApp.app, DetectMode.ASF_DETECT_MODE_IMAGE, DetectFaceOrientPriority.ASF_OP_0_ONLY,
                    1, FaceEngine.ASF_FACE_RECOGNITION);
            if (res == ErrorInfo.MOK) {
                boolean success = FaceServer.getInstance().registerNv21(BaseApp.app, nv21.clone(), previewSize.width,
    ...
    public void destroy() {
        unInit();
        if (faceHelper != null) {
            ConfigUtil.setTrackedFaceCount(BaseApp.app.getApplicationContext(), faceHelper.getTrackedFaceCount());
            faceHelper.release();
            faceHelper = null;
        }
        ...
        if (faceHelper != null) {
            ...
            if (registerStatus == REGISTER_STATUS_READY && !facePreviewInfoList.isEmpty()) {
                ...
                if (facePreviewInfo.getMask() != MaskInfo.WORN) {
                    ...
                    Toast.makeText(BaseApp.app, "注册照要求不戴口罩", Toast.LENGTH_SHORT).show();
                    updateRegisterStatus(REGISTER_STATUS_DONE);
                }
            }
    ...
    public Point loadPreviewSize() {
        String[] size = ConfigUtil.getPreviewSize(BaseApp.app).split("x");
